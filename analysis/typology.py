...
quad = {("high", "high"): "bridge (import high, export high)", ("low", "high"): "home-grown, exported", ("high", "low"): "imported, grown inside", ("low", "low"): "inward (home-grown, kept inside)"}
rows = []
for s, r in E.items():
    im, ex = float(r["import_share_early"]), float(r["export_share_median"])
    q = ("high" if im >= im_med else "low", "high" if ex >= ex_med else "low")
    f = FS.get(s); fs = f and np.nanmedian([float(f[k]) for k in ("y0", "y2", "y4") if f.get(k) not in ("", "nan")] or [np.nan])
...
with (load.OUT / "typology.csv").open("w", newline="") as fh:
    w = csv.writer(fh); w.writerow(["sub", "name", "band", "papers", "import_share", "export_share", "type", "founder_share_y0_4"]); w.writerows(rows)
